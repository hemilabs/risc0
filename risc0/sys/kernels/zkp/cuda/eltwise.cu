...
__global__ void eltwise_zeroize_fp(Fp* elems, const uint32_t count) {
  uint idx = blockIdx.x * blockDim.x + threadIdx.x;
  // Bounds check (as in every other eltwise kernel): the launch rounds the grid
  // up to a multiple of the block size, so without this guard threads with
  // idx >= count read AND write up to blockDim-1 elements PAST the buffer, into
  // adjacent pool-reused memory — corrupting other buffers (intermittent invalid
  // multi-segment proofs) and reading uninitialized memory (initcheck flag).
  if (idx < count) {
    Fp val = elems[idx];
