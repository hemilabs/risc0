...
#ifdef __HIPCC__
// RDNA4 (gfx1201): Reduce thread count to decrease total scratch working set.
// With 256 threads: 8 waves/block × 6KB scratch/thread = 1.5MB/block → L0/L2 thrashing.
// With 128 threads: 4 waves/block × 6KB scratch/thread = 768KB/block → better cache reuse.
// amdgpu_waves_per_eu(1,4): allows compiler to use more VGPRs per wave, reducing spills.
...
__attribute__((amdgpu_flat_work_group_size(EVAL_CHECK_THREADS, EVAL_CHECK_THREADS)))
#endif
